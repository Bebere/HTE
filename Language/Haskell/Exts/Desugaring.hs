...
instance Desugar Pat where
  ...
  -- HSE
  desugar (PTuple []) = desugar $ PApp (Special UnitCon) []
  desugar (PTuple [p])= desugar p  
  desugar (PTuple pats)= 
...
instance Desugar Decl where
  ...
  desugar d@(SpecSig          src qName ts) = error "Not supported!"
